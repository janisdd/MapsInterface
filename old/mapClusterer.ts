...
interface _AbstractClusterer {
  ...
  */
  minMarkersPerCluster: number

  /**
  * the inclusive max amount of markers to build a cluster
  */
  ...
  /**
  * the inclusive max distance a other marker can be distant from the initial cluster marker
  * in order to be added to the same cluster
  */
  maxDistanceForClusterInPx: number
...
class _Clusterer { //implements AbstractClusterer but as static members
  ...
  * the inclusive min amount of markers to build a cluster
  */
  static minMarkersPerCluster: number = 2

  /**
  * the inclusive max amount of markers to build a cluster
  */
  static maxMarkersPerCluster: number = 1000000
  ...
  * in order to be added to the same cluster
  */
  static maxDistanceForClusterInPx = 100
  ...
  private static getClustersFromPairs(pairs: MarkerPair[]): Cluster[] {
    ...
    for (let i = 0; i < sortedPairs.length; i++) {
        ...
        //when pair is null this marker/pair is already in a cluster
        if (pair === null) continue

        //remove the current marker from the list because this marker will be definately in a cluster
        sortedPairs[i] = null
  ...
  private static getCluster(pair: MarkerPair, sortedPairs: MarkerPair[]) : Cluster {
    ...
    //only build a cluster when we have
    if (cluster.markers.length > 1 &&
      cluster.markers.length >= Clusterer.minMarkersPerCluster && cluster.markers.length <= Clusterer.maxMarkersPerCluster) {
